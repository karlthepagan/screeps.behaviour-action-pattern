var Setup = function(typeName){
    ...
    this.isValidSetup = function(room){
        if( room.controller.level < this.minControllerLevel ) {
            return false;
            if (DEBUG && (Memory.debug.setup || this.type === Memory.debug.setupType)) console.log(room.name, this.type, 'low RCL');
        }
        ...
        if( room.remainingEnergyAvailable < minAbsEnergyAvailable ||
            ...
            if (DEBUG && (Memory.debug.setup || this.type === Memory.debug.setupType)) console.log(room.name, this.type, 'not enough energy');
            return false;
        }
        ...
        if( maxCount == 0 || maxWeight == 0 ) {
            if (DEBUG && (Memory.debug.setup || this.type === Memory.debug.setupType)) console.log(room.name, this.type, 'zero count || weight');
            return false;
        }
        if( maxCount == null )
            maxCount = Infinity;
        ...
        if (DEBUG && (Memory.debug.setup || this.type === Memory.debug.setupType)) console.log(room.name, this.type, 'count:',
